...
class FinancialSummary extends Page implements HasForms
{
        ...
        return $payments->groupBy('feeType.name')->map(function ($group) {
            return [
                ...
                'collection_rate' => round(
                    ($group->where('status', 'verified')->unique('family_id')->count() / 
                    max(1, Family::where('status', 'active')->count())) * 100, 1
                ),
            ];
        });
    }
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Action::make('back_to_list')
                ...
                ->color('gray')
                ->url(fn (): string => FinancialReportResource::getUrl('index')),
        ];
    }
    ...
    private function downloadReport()
    {
        ...
        $pdf = \PDF::loadView('pdf.financial-summary-report', [
            'summary' => $summary,
            'payments' => $payments,
            'monthly' => $monthly,
        ...
        $filename = 'laporan-keuangan-perumahan-' . 
